...
async fn main() {
    ...
    plugin::on_group_msg(move |event| {
        ...
        async move {
            // 处理聊天命令
            process_chat_command(event, chat, config, messages).await;
        }
    });
}
...
async fn process_chat_command(
    ...
    let img = match gen_img(&md).await {
        Ok(v) => v,
        Err(e) => {
            error!("{}", e);
            event.reply("生成图片失败");
    ...
    let text_seg = Segment::new("node", json!({ "content": [text_seg] }));
    let img_seg = Segment::new("node", json!({ "content": [img_seg] }));

    let msg = Message::from(vec![img_seg, text_seg]);

    event.reply(msg);
}

async fn gen_img(md: &str) -> Result<Vec<u8>> {
