...
pub const CF_HELP: &[&str] = &[
    ...
    concat_with::concat_line!(
        ...
        "",
        "默认参数是/cf recommend moderate -c 1",
        "例如：",
...
pub static DUEL_HELP: LazyLock<Value> = LazyLock::new(|| {
    json!([
        ...
        },
        {
            "type": "text",
            "data": {
                "text": concat_with::concat_line!(
                    "duel challenge @p rating [tags]：挑战用户p，题目将随机选取一道分数为 rating，标签为 [tags] 的题目",
                    "",
                    r#"标签的用法为：输入 CF 题目中包含的标签，多个标签用空格隔开，本身包含空格的标签请将空格用下划线 "_" 替换。可以在标签前加上 "!" ，表示要求不包含该标签。"#,
                    "",
                    "例如：",
